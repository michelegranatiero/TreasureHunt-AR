...
import com.example.treasurehunt_ar.ui.utils.AppViewModel.Companion.ERROR_TAG
...
fun AuthenticationButton(buttonText: Int, onRequestResult: (Credential) -> Unit) {
    ...
    Button(
        ...
        // onClick = { coroutineScope.launch { launchCredManBottomSheet(context, onRequestResult = onRequestResult) } },
        modifier = Modifier
            .fillMaxWidth()
            .padding(16.dp, 0.dp)
    ) {
...
private suspend fun launchCredManButtonUI(
    ...
    try {
        ...
        SnackbarManager.sendEvent(event = SnackbarEvent(
            message = UiText.StringResource(R.string.no_accounts_error)
        ))
...
suspend fun launchCredManBottomSheet(
    ...
    try {
        val googleIdOption = GetGoogleIdOption.Builder()
            ...
            // .setAutoSelectEnabled(true)  // auto select the first account (automatic sign in)
            // .setNonce("")   // avoid replay attacks
